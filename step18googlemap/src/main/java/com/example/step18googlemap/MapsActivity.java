...
//     */
    //GoogleMap 이 준비완료되면 호출되는 메소드
    @Override
    public void onMapReady(GoogleMap googleMap) {
        mMap = googleMap;

        // 카메라가 비출 위치 (위도, 경도)
        // 고등동 우체국 37.42788032236273, 127.10239180082979
        //위도 경도 객체에 위치를 표시할 위도 경도를 생성자의 인자로 전달
        LatLng location = new LatLng(37.42788032236273, 127.10239180082979);
        //지도상에 표시할 마커의 옵션 객체
        MarkerOptions options=new MarkerOptions();
        options.position(location);
        options.title("고등동 우체국");
        ...
        //카메라 이동하기 위한 객체를 이용해서 실제로 애니메이션 효과와 함께 이동하기
//      mMap.animateCamera(cu);
        mMap.moveCamera(cu);//애니메이션 없이 이동
    }
