...
public class DetailActivity extends AppCompatActivity implements Util.RequestListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //dto 라는 키값으로 남긴 Serialzable type 객체(MemberDto 객체) 의 참조값 얻어오기
        ...
        updateBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //웹서버에 요청할 url
                String url="http://211.250.184.112:8888/spring04/api/member/update.do";
                //요청 파라미터를 Map에 담는다
        ...
        deleteBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //삭제할 회원의 번호
                int num=dto.getNum();
                String url="http://211.250.184.112:8888/spring04/api/member/delete.do";
                //요청 파라미터를 Map에 담는다
                Map<String, String> params=new HashMap<>();
                params.put("num",Integer.toString(num));
                //Utill 을 이용해서 전송
    ...
    public void onSuccess(int requestId, Map<String, Object> result) {
        ...
        if(code != 200){//응답코드가 200이 아니면 실패이다
        ...
        new AlertDialog.Builder(this).setTitle("알림").setMessage(msg)
                .setNeutralButton("확인", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        //DetailActivity finish() 메소드를 호출해서
                        //DetailActivity 가 종료되도록 한다
                        //결과적으로 onStop() 까지 호출되어서 비활성화된 상태로 대기중이던
                        //MainActivity 가 onRestart() onStart() onResume() 이 호출되면서
                        //다시 활성화가 된다
                        //따라서 onStart() 메소드 안에서 회원 목록을 서버에 다시 요청하게
                        //되기 떄문에 업데이트된 회원 목록을 볼 수 있는 것이다
                        finish();
